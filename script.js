...
const editions={waves:{image:'assets/images/waves-front.webp',alt:'Blue Waves Prime Series notebook standing against a white background',label:'Waves / Mineral blue',name:'Waves',amazon:'https://www.amazon.in/Pebbles-Threads-Notebook-Sketching-Painting/dp/B0H8MP3RLS'},geometric:{image:'assets/images/geometric.webp',alt:'Brown Geometric notebook with concentric black linework',label:'Geometric / Terracotta',name:'Geometric',amazon:'https://www.amazon.in/Pebbles-Threads-Notebook-Sketching-Geometric/dp/B0H8MHGXTL'}};
$$('[data-select-edition]').forEach(button=>button.addEventListener('click',()=>{const key=button.dataset.selectEdition;const image=$('[data-hero-image]');document.body.dataset.edition=key;$('[data-amazon-link]').forEach(link=>link.href=editions[key].amazon);$('[data-amazon-label]').forEach(label=>label.textContent=editions[key].name);image.style.opacity='0';setTimeout(()=>{image.src=editions[key].image;image.alt=editions[key].alt;$('[data-edition-label]').textContent=editions[key].label;image.style.opacity='1';window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})},250)}));

$('[data-amazon-track]').forEach(link=>link.addEventListener('click',()=>{if(window.FS&&typeof window.FS.event==='function')window.FS.event('Amazon Listing Opened',{edition_str:link.dataset.amazonEdition||document.body.dataset.edition,placement_str:link.dataset.amazonPlacement||'unknown'})}));
